...
from src.zen_db_Betrive import insert_betrive
            ...
            for msg_id in msg_ids[:3]:
                ...
                for response_part in msg_data:
                    ...
                    if isinstance(response_part,tuple):
                        ...
                        unix_micro = int(mail_date.timestamp() * 1_000_000)
                        ...
                        print(f"【件名】: {subject}")
                        print(f"【受信日時(JST)】: {mail_date}")
                        print(f"【本文】:\n{body}")
                        print("-" * 50)

                        # データベースに登録
                        insert_betrive(
                            int(msg_id.decode('utf-8')),  # 数値 (28249)
                            unix_micro,  # 数値 (100ナノ秒ではなく、普通のUNIXマイクロ秒)
                            CPI_EMAIL_USER,  # 文字列
                            subject  # 文字列
                        )
                        ...
                        # 今回処理した「最大3通」の中の、一番最後のメールIDを保存して次回に備える
                    if msg_ids:
